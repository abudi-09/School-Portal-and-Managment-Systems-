...
const Scores = () => {
  ...
  const averageRank = Math.round(
    currentData.subjects.reduce((sum, s) => sum + s.rank, 0) /
  ...
  const computeStats = (subjects: { total: number; rank: number }[]) => {
    if (!subjects || subjects.length === 0)
      return { totalScore: 0, averageScore: "0.0", averageRank: 0 };
    const total = subjects.reduce((s, x) => s + (x.total || 0), 0);
    const avg = (total / subjects.length).toFixed(1);
    const avgRank = Math.round(
      subjects.reduce((s, x) => s + (x.rank || 0), 0) / subjects.length
    );
  ...
  const combined = {
    ...
    averageScore: (
      (parseFloat(sem1.averageScore) + parseFloat(sem2.averageScore)) /
      2
    ).toFixed(1),
          ...
          >
            <div>
              <div className="flex items-center gap-2">
                ...
                <Badge
                  variant={getGradeColor(subject.grade)}
                    ...
                    <tbody className="bg-card divide-y divide-border/50">
                      {Object.entries(subject.breakdown).map(
                        ([type, score]) => (
                          ...
                          >
                            <td className="px-6 py-4 whitespace-nowrap text-foreground capitalize">
                              {type}
                            </td>
            ...
            <Card className="transition-shadow hover:shadow-md">
              ...
              <CardContent>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <StatCard
                    label="Total Score"
                    value={sem1.totalScore}
                    icon={Target}
                    subtitle={`Out of ${
                      semesterData.semester1.subjects.length * 100
                    }`}
                    variant="info"
                  />
                  <StatCard
                    label="Average Score"
                    value={sem1.averageScore}
                    icon={Award}
            ...
            <Card className="transition-shadow hover:shadow-md">
              ...
              <CardContent>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <StatCard
                    label="Total Score"
                    value={sem2.totalScore}
                    icon={Target}
                    subtitle={`Out of ${
                      semesterData.semester2.subjects.length * 100
                    }`}
                    variant="info"
                  />
                  <StatCard
                    label="Average Score"
                    value={sem2.averageScore}
                    icon={Award}
                    subtitle="Average across subjects"
                    variant="success"
                  />
            ...
            <Card className="transition-shadow hover:shadow-md">
              ...
              <CardContent>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <StatCard
                    label="Total Score"
                    value={combined.totalScore}
                    icon={Target}
                    subtitle={`Out of ${
                      (semesterData.semester1.subjects.length +
                        semesterData.semester2.subjects.length) *
                      100
                    }`}
                    variant="info"
                  />
                  <StatCard
                    label="Average Score"
                    value={combined.averageScore}
                    icon={Award}
                    subtitle="Average of both semesters"
                    variant="success"
                  />
                  <StatCard
                    label="Average Rank"
                    value={combined.averageRank}
                    icon={TrendingUp}
                    subtitle="Combined class position"
                    variant="default"
                  />
                </div>
              </CardContent>
