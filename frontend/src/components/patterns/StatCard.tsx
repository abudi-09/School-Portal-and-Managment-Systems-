...
interface StatCardProps {
  label: string;
  value: string | number;
  // `icon` may be provided as a Lucide icon component (e.g. `Clock`) or
  // as an instantiated element (e.g. `<Clock />`). Accept either form.
  ...
  return (
    <Card
      ...
      aria-label={`${label}: ${value}${
        trend
          ? `, trending ${trend.positive ? "up" : "down"} by ${trend.value}`
          : ""
      }`}
    >
      <CardContent className="p-6">
        <div className="flex items-center justify-between gap-4">
          ...
          >
            {
              // If an icon component was passed (function), render it with sizing props
              // Otherwise if an element was passed, clone it to add sizing class
              typeof Icon === "function"
                ? // Lucide icons are functions/components
                  // eslint-disable-next-line react/jsx-pascal-case
                  (Icon as any)({ className: "h-6 w-6" })
                : isValidElement(Icon)
